...
1. Model loading in lifespan, not at import time.
   ...
   Concretely, startup builds the classical reference gallery (~67 PNGs →
   HSV histograms + ORB descriptors), loads the ONNX embedding backbone into
   an ``InferenceSession``, and builds the prototype embedding gallery — with
   a graceful fallback to the classical identifier if the ONNX file is absent.
...
async def lifespan(application: FastAPI):
    ...
    Startup loads three shared resources onto ``application.state``:

    1. The classical reference gallery — ~67 card-art PNGs precomputed into
       HSV histograms + ORB descriptors (Stage 2 classical arm, kept as a
       fallback / selectable baseline).
    2. The ONNX embedding backbone — an ``onnxruntime.InferenceSession``
       loaded once via ``get_onnx_embedder()`` (CPU provider, no torch).
    3. The prototype embedding gallery — every reference PNG embedded and
       mean-pooled into per-card prototypes; the embedding-NN identifier
       built from it is the default served identifier.
    ...
    # Build the Stage 2 embedding gallery (onnxruntime, CPU, no torch).
