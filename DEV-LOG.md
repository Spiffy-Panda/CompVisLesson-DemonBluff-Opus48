...
**Options considered:** (a) keep per-action PNG bursts — no temporal continuity, and the grab-per-action cadence is exactly why the crop miner had to skip tinted flip bursts; (b) OBS/external recorder — heavy dependency, extra encode hop, no programmatic sidecars (already rejected for capture in the 2026-07-29 WGC entry); (c) in-process WGC → cv2.VideoWriter recorder — chosen.

**Choice:** New util `utils/python/record_session.py` (straight to utils — dataset collection depends on it immediately, so Rule 1's trigger was met at birth). Reuses the hardened window-finding/exe-verification/DPI code from the frame grabber; that grabber was simultaneously promoted (`scrap_scripts/python/04_grab_frame.py` → `utils/python/grab_frame.py`; plain move, not `git mv` — scrap is gitignored so the file was never tracked). Both cataloged in `utils/README.md`.

**Design — threaded writer, CFR output:** the WGC `on_frame_arrived` callback never touches disk: it copies the BGRA buffer and `put_nowait`s onto a bounded queue (queue-full frames are counted and reported as drops); a dedicated writer thread encodes. The writer **resamples to true constant frame rate** (`--fps`, default 30): WGC delivery is variable (~55 FPS, gapped when the compositor idles), and Stage 0's stride decode derives its stride from the container's declared fps — a VFR-ish stream in a fixed-fps container drifts wall-clock time away from frame-index time over a 30–60 min session. One frame is emitted per 1/fps monotonic tick — duplicating the last captured frame across capture gaps, dropping extras when capture outpaces the target — so `frame_index = round((wall_ts - cfr_epoch_wall) * target_fps)` holds by construction (`cfr_epoch_wall` recorded in `session.json`; a video plus a synchronized event log is self-labeling data). Resolution is read from the first arriving frame (never hard-coded); a mid-session resize closes the segment and opens `session_seg2.mp4` rather than corrupting the stream. Sidecars: `frames.jsonl` (per encoded frame: index, ideal CFR timestamp, source-frame monotonic + wall timestamps, dup flag) and `session.json` (game version — `--game-version` is a required flag per the footage-version-drift rule — resolutions, target/measured fps, codec, duplicate/drop counts, segments).

**Codec fallback result (this box):** `avc1` fails to open — opencv-python's bundled FFmpeg wants an OpenH264 DLL (`openh264-1.8.0-win64.dll`) that isn't present — so the `mp4v` fallback engages. MPEG-4 Part 2 at default quality is bulkier/softer than H.264; acceptable for dataset capture, revisit (e.g. drop the DLL next to the venv's cv2) if size becomes a problem. ffprobe is not installed here, so CFR correctness is proven by construction + self-test, not container probing.

**Testing:** `scrap_scripts/python/17_record_session_selftest.py` (synthetic BGRA frames, no game): 27/27 checks pass — mid-stream resize → two decodable segments with exact frame counts; jsonl/session.json consistency; CFR arithmetic (steady 2 s @ 60 FPS capture → 61 encoded @ 30 fps, zero dups; a 1 s capture gap → same count, ~29 duplicates, final ideal timestamp still at capture end — no skew); queue-full drop counting; `--max-fps` thinning. **Live smoke test: the game was NOT running** (the recorder correctly rejected a File Explorer window titled with the install path — the exe-verification working as designed), so measured live FPS / GB-per-hour numbers are still pending the first real session.

**Notes / risks:** The project `.venv` lacked the capture stack — the earlier collect sessions ran under the global Python 3.14 (flagged in the 2026-07-29 eval entry as "worth being deliberate about"); `pywin32==312` and `windows-capture==2.0.0` are now installed **into the venv** (matching RESEARCH.md's validated version) so the standard interpreter runs the capture utils. The tracked `requirements.txt` was NOT edited (it documents the runtime/dev stack; whether capture-box-only deps belong there is an open question for the user — flagged, not decided). Disk-rate estimate pending the first real recording; mp4v at 1280×720@30 typically lands in the low single-digit GB/hour. The mp4 container's declared fps equals `--fps` and the stream is CFR, so `frame_select.py` needs no changes.

## 2026-07-29 — `eval_04` on `collect_03` (fixed pipeline, harder sample) + `live_crops_v1` round-2 dataset + Night Cycle / disguise findings

**Context:** With the HUD/gate/ensemble fixes from the eval-wave entry below already shipped, this session ran the fixed pipeline against a third, larger, in-progress live collection (`collect_03`, still being actively played by a concurrent play/labeling agent) to check whether those fixes hold up on new content, and in parallel mined a first labeled live-crop dataset for a round-2 embedding fine-tune. Both pieces share this entry since the dataset was built read-only off the eval's own ground truth.

**Choice — `eval_04`:** New scrap runner `scrap_scripts/python/11_eval_collect03.py` + `12_score_collect03.py`, evaluated against a **frozen snapshot** of `collect_03` (`frame_snapshot.txt`, 352 frames — `collect_03/` itself was still being appended to live and was opened strictly read-only) across **3 arms** — classical, embedding, ensemble. Ground truth: `ground_truth.json`, **29 hand-verified frames / 209 hand-labeled non-facedown card slots** (built and visually cross-checked by the eval agent, spanning hexagon through decagon boards, Ascension 2-4, and every frame category the taxonomy tracks — full_reveal, partial_reveal, post_execution, loss_reveal, night_kill, facedown_boundary). Outputs at `scrap_scripts/python/out/eval_04/` (`scoring_report.json`, `per_role_table.csv`, `run_log.txt`, `summary.json/csv`).

**Results — HUD fix holds at scale:** 0.0% false-positive rate in all three arms across all 344 board frames (both the top-left text zone and the top-right badge zone) — the Fix-1 HUD zones from the eval-wave entry below generalize cleanly to a new, larger, later collection, not just the frames they were tuned against.

**Results — tint guard's suppression is embedding-only, a nuance for a future pass:** measured mean confident-card count per board frame, tinted vs. non-tinted (28 tinted / 324 non-tinted of the 352 frames, `measure_red_shift` scores 22.9–31.5 tinted vs. -16.0 to 5.1 non-tinted — the gap still holds cleanly): classical 3.64 vs. 3.82 (barely moved), **embedding 0 vs. 2.49 (fully silenced)**, ensemble 3.86 vs. 4.75. The `×0.7` discount + 0.5 floor (the eval-wave entry's Fix-1 tint guard) is identifier-agnostic by design, but in practice it only bites embedding: embedding's raw confidence scale (~0.12–0.40, per the ensemble sub-entry below) essentially never clears 0.5 after any discount, tinted or not, while enough of classical's raw confidences sit ≥0.71 (the threshold needed to survive `×0.7` and still clear the 0.5 floor) that tinting barely dents it. Not a regression — the guard was built to be a floor, not a ceiling — but worth flagging: classical identifications made under Kill-Mode tint get materially less discounting in practice than the guard's intent suggests.

**Results — accuracy, harder sample, same ordering as the prior eval:** hit rate over the 209 scoreable ground-truth slots: **ensemble 43.5% (91/209)**, classical 29.7% (62/209), embedding 28.7% (60/209) — ensemble still wins, but all three numbers sit well below the eval-wave entry's 6-frame spot-check (classical 50.0%, embedding 52.9%, ensemble 64.7%), because this is a much larger, harder, more representative sample (29 hand-verified frames across every board shape and frame category, not a hand-picked "known fully revealed" subset).

**Results — reveal-only identity set is still essentially unscored:** the 7 identities that only ever show their true art after an execute/loss reveal (Minion, Puppet, Puppeteer, Witch, Chancellor, Lilis, Baa — 29 of the 209 GT slots) scored **0/29 in classical**, and **1/29 in embedding and ensemble** (a single Baa hit — 25% of Baa's own 4 instances; every other identity in the set is still 0% in all three arms). Direct motivation for `live_crops_v1`'s reveal-only mining below.

**Results — embedding "Hunter attractor":** the embedding confusion matrix shows 7 *distinct* roles (Minion, Witch, Judge, Architect, Oracle, Druid, Lilis) all misidentified as **Hunter** — one over-generalized prototype absorbing a wide spread of unrelated identities, not a handful of pairwise mix-ups. Classical's confusions are pairwise instead and dominated by two specific clusters: `Slayer→Bombardier` (10 of classical's 58 wrong misses) and `Hunter→Poisoner` (10) — both carried into the ensemble's confusion table too, since ensemble adopts classical's answer whenever embedding abstains.

**Results — run-summary fade screens are still ungated:** the `run_summary` gate-taxonomy frames (154, 313 — the score/summary fade-out between villages) classify as `board` in all three arms, the same gap already logged for win/reward/objective/menu/tutorial dialogs in the eval-wave entry below. One concrete cost: frames 312/313 hallucinate `Baa@0.14` in both embedding and ensemble — a fade-screen artifact, not a real card.

**Results — totem phantom-box discovery:** this same `eval_04` run is what first found the two background-totem phantom card boxes; the fix (new `HUD_ZONES` entries + widened badge zone) is already logged separately as today's "Background-totem phantom-box fix" entry above (commit `da6e8d8`) and is not repeated here.

**Choice — `live_crops_v1`:** New scrap miner `scrap_scripts/python/13_mine_live_crops.py`, built **read-only** against `eval_04`'s frozen `collect_03` snapshot plus `collect_01`/`collect_02`, producing a labeled crop dataset for a round-2 embedding fine-tune on the user's Titan Xp machine (transfer artifact `scrap_scripts/python/out/live_crops_v1.zip`, ~5.2 MB). **264 crops, 33 identities**, three priority-ordered provenance sources when a `(collection, frame, slot)` key collides: (1) `ground_truth.json`'s 199 hand-verified crops, always wins; (2) `reveal_moment` (16 crops) — conservative regex mining of `actions.md`'s "flipped to X" execute-reveal lines, the dataset's main source for the reveal-only identity set; (3) `actions_log` (49 crops) — the same conservative join over ordinary `Card #N revealed: IDENTITY` lines, `collect_01`/`02` only (`collect_03`'s log batches multi-card reveals without per-card coordinates, so it's skipped there as not unambiguous enough to harvest conservatively). Every crop is labeled with the identity **displayed** in that exact frame, never the eventual true identity — matching what an embedder actually observes; `true_identity` is carried separately for disguise-transition pairs, never as an alternative training label.

All 7 reveal-only identities are present but thin for 3: **Chancellor (1), Witch (3), Lilis (3)** — the ones most worth targeting in a future collection pass.

**A real bug found and fixed during the mining/QA pass:** the miner's first version assumed identity is invariant across a multi-frame `actions.md` log cycle. Two things broke that on closer inspection: (1) a frame's own localizer box **count** is not a reliable board-shape signal — a just-executed card's skull overlay can drop a box, silently sending a coordinate lookup to the wrong shape's slot table and cropping the *neighboring* card (fixed: `infer_board_shape()` now matches the full per-slot coordinate fingerprint of a shape against the frame's actual boxes, not box count); (2) a flip sometimes only settles part-way through a multi-frame burst (`collect_03` frame 305 still shows the pre-flip disguise while 306, the very next grab, shows the true art) — an automated HSV hue/saturation same-card verifier was tried and **rejected** (Kill-Mode's tint shifts hue enough that even genuine matches correlate poorly), so the fix was the conservative one: only the **last** frame cited in a multi-frame cycle is ever used. Net effect: this dataset has **zero Kill-Mode-tinted crops** by construction — correct for label safety, but a real coverage gap for round-2 (see below).

**Known gaps:** zero Kill-Mode-tinted crops (above); **11 identities still cropless** — Alchemist, Baker, Bard, Doppelganger, Drunk, Lycanthrope, Plague_Doctor, Pooka, Werewolf, Witness, and Twin_Minion (folded into the `Minion` alias, so this just means the literal string "TWIN MINION" never appeared in an unambiguous reveal line this run).

**Fine-tune brief for the Titan Xp round-2 pass, priority order:** (1) the reveal-only identity set's displayed art (Minion, Puppet, Puppeteer, Witch, Chancellor, Lilis, Baa) — 0–3% today, the clearest ceiling in the whole eval; (2) Hunter-attractor hard negatives — pair embedding's false-Hunter hits (Minion, Witch, Judge, Architect, Oracle, Druid, Lilis) against genuine Hunter crops so the prototype stops absorbing unrelated roles; (3) `Hunter↔Poisoner` and `Slayer↔Bombardier` hard-negative pairs, the two largest classical confusion clusters found this eval.

**Why:** same reasoning as the eval-wave entry below — measure honestly against real footage before assuming a fix generalizes, and don't tune on the same frames used to evaluate. `eval_04` is the first check of the eval-wave fixes against genuinely new content (not `collect_02` again), and `live_crops_v1` is the first labeled live-capture dataset this project has had for any embedding fine-tune, sourced from three separate collection sessions rather than the single-session samples every prior eval used.

**Notes / risks:** No `src/` changes in this half of the session (per `live_crops_v1`'s brief — a concurrent play/labeling agent was using `collect_03` and `eval_04`'s outputs, both opened read-only throughout; the totem-box fix that *did* touch `src/` is the separate entry above). `scrap_scripts/` remains gitignored and unpromoted (Rule 1's promotion trigger still not met — flagged again). The reveal-only-set and Hunter-attractor findings here are eval evidence for the round-2 fine-tune, not yet acted on in `src/dbcv/identify.py` or `embed.py`.
