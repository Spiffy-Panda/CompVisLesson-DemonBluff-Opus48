...
**Badge blob-detection does not work as a primary anchor.** The numbered position badges (`#1`, `#2`, ...) on each card are art-independent UI chrome and were the first candidate for a reliable geometric anchor. In the spike, direct blob detection on the badge regions over-fired at 30–60 detections per frame because the card's clue/ability text panels produce blobs that are visually indistinguishable from the badge blobs at the segmentation stage. Badges remain useful for *ordering* already-detected boxes (by running targeted OCR on the `#` glyph), but they are not a viable primary localization anchor. This finding is noted in `DEV-LOG.md` (2026-06-22) and in `knowledge-base/lessons/observed-board-layout.md` as a correction to the earlier "badges are ideal landmarks" note. (Flag: the knowledge-base file calls badges "ideal landmarks" without the blob-detection caveat — this desync has been flagged but not yet corrected as of this module's authoring.)

**Board-vs-modal state gate is weak.** The current implementation has no reliable gate to distinguish a clean board frame from a frame where a modal dialog occludes part of the board. During the spike, a center-brightness heuristic (bright center = board) misread dark-background modal dialogs as board frames because *Demon Bluff*'s modals have bright art and text on a dark field — the heuristic fires in exactly the wrong direction. On modal frames, the classical localizer returns a small number of boxes (0–2) rather than a full ring count, which is incorrect behaviour. The correct fix is a frame-state classifier (board / menu / modal) — this is Stage 0's main remaining problem. It is forward-referenced to the frame-selection module in the intended course arc. Until it is in place, callers should not interpret "fewer than expected boxes" as evidence that the board is empty.
