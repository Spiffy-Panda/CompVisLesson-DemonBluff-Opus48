...
| 2026-06-22 | Stage 3 embedding-NN built (frozen MobileNetV3-Small → ONNX/CPU, cosine-NN over a re-embeddable gallery) — but **a frozen ImageNet backbone does NOT beat classical** here | The backbone collapses cartoon characters into one cluster (cosine 0.65–0.94); it overidentifies rather than improving correctness. The architecture is right; **fine-tuning on card crops is the real fix** (now feasible on the Titan Xp). Default identifier flagged for Panda (recommend conservative classical until fine-tuned) | refines the 2026-06-21 embedding-NN target (frozen alone is insufficient) |
