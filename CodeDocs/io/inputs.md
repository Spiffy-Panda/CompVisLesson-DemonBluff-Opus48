...
| Card-art gallery | `knowledge-base/card-art/<role_class>/<identity>/<file>.png` (gitignored) | PNG image files | **Active — Stage 2 + Stage 3.** 67 PNGs across 43 townee identities. Consumed by both `build_gallery()` (HSV+ORB) and `build_embedding_gallery()` (ONNX embeddings) at server startup. In-memory only; never persisted. Art swap = replace PNGs + restart; zero gradient steps for either gallery. See `CodeDocs/sources/dbcv/gallery.md`. |
| ONNX embedding backbone | `models/mobilenetv3_small_embed.onnx` (gitignored via `*.onnx`) | ONNX model file | **Active — Stage 3.** MobileNetV3-Small with classifier head stripped; input [N,3,224,224] float32; output [N,576] float32 (raw, L2-normalised at runtime). Generated by `utils/python/export_backbone.py` (dev-only, requires torch). Loaded once at startup by `OnnxEmbedder` via onnxruntime CPU provider. See `models/README.md` for regeneration. |
