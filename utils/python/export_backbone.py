...
ROLE: this generates the FROZEN ImageNet BASELINE
(models/mobilenetv3_small_embed_frozen.onnx), used only for the head-to-head
comparison in the lesson plan.  The SERVED model is the domain-fine-tuned
backbone produced by utils/python/finetune_embedding.py.
...
3. Exports to ONNX (opset 13) at models/mobilenetv3_small_embed_frozen.onnx.
...
# Frozen ImageNet BASELINE (not the served model — that is the fine-tuned backbone
# produced by utils/python/finetune_embedding.py).
_ONNX_PATH = _MODELS_DIR / "mobilenetv3_small_embed_frozen.onnx"
...
def export_to_onnx(embedder: MobileNetV3SmallEmbedder) -> Path:
    """Export the embedder to ONNX at models/mobilenetv3_small_embed_frozen.onnx.
...
def main() -> None:
    ...
    print()
    print("This is the FROZEN baseline; the SERVED model is regenerated by")
    print("utils/python/finetune_embedding.py.  Both .onnx are gitignored; see models/README.md.")
    print("=" * 60)
