...
- [PLAN-pipeline](plans/PLAN-pipeline.md) — video → game state → REST, in 6 stages. **Shipped 2026-06-22 (in `src/dbcv/`):** Stage 0 frame-state gate, Stage 1 localization, Stage 2 identification (classical + a **domain-fine-tuned** embedding-NN served via ONNX-CPU, adopted as default with a top1−top2 margin gate), the REST slice (`POST /v1/snapshot`), Stage 5 ONNX serving, and a `utils/` CLI runner — **81 tests green**. **Remaining:** Stage 3 on-card OCR, Stage 4 temporal assembly; optional round-2 dataset-building (real mined+labeled crops) to deepen Stage 2.
