...
**Where it stands:** Stages 0, 1, 2 + the REST service + a CLI runner are shipped (classical/CPU, plus a **domain-fine-tuned** embedding backbone served via ONNX-CPU). **81 tests pass (~24 s).** `torch 2.7.1+cu118` + `onnxruntime` installed; GPU verified on the Titan Xp. The Stage-2 embedding fine-tune (round 1) is **done and adopted as the default identifier** (Proxy-Anchor LP-FT fixed the collapse: inter-prototype cosine 0.85 → 0.41; abstains on a top1−top2 margin). Authoritative history is in `DEV-LOG.md` (newest first); decisions in `PROJECT-PITCH.md`; lesson plan is **8/10 modules** authored.
...
**Independent work ready to pick up (no decision needed):** Stage 0 stride-decode + perceptual-hash dedup; Stage 3 on-card OCR (closed-vocab recognizer); Stage 4 temporal smoothing; lesson Modules 05 (reflect the fine-tune arc — being synced this session), 06 (OCR), 07 (assembly). **Round-2 dataset-building** (mine + label real board crops) is the natural follow-on to deepen Stage 2.
